...
export class HttpService {
  ...
  postHideData(headerName: string): Observable<string> {
    let resp: Observable<string> = null;
    resp = this.http.get<string>(
      `http://localhost:30285/Student/PostHideData?headerName=${headerName}`
    );
    return resp;
  }
  ...
  postShowData(headerName: string): Observable<string> {
    let resp: Observable<string> = null;
    resp = this.http.get<string>(
      `http://localhost:30285/Student/PostShowData?headerName=${headerName}`
    );
    return resp;
  }
  ...
  getPaginatedData(
    ...
    return (resp = this.http.get<Student[]>(
      `${this.url}/GetPaginatedData?studentId=${studentId}&filter=${filter}&sort=${sortDirection}&pageIndex=${pageIndex}&pageSize=${pageSize}`
    ));
  }
